...
func parseSettingsUpdateRequest(req map[string]any) (*config.AdminConfig, *config.RuntimeConfig, *config.ResponsesConfig, *config.EmbeddingsConfig, *config.AutoDeleteConfig, *config.CurrentInputFileConfig, *config.ThinkingInjectionConfig, *config.OutputIntegrityConfig, map[string]string, error) {
	...
	if raw, ok := req["output_integrity_guard"].(map[string]any); ok {
		cfg := &config.OutputIntegrityConfig{}
		if v, exists := raw["enabled"]; exists {
			b := boolFrom(v)
			cfg.Enabled = &b
		}
		...
		outputIntegrityCfg = cfg
	}

	return adminCfg, runtimeCfg, respCfg, embCfg, autoDeleteCfg, currentInputCfg, thinkingInjCfg, outputIntegrityCfg, aliasMap, nil
}
